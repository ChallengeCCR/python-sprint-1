package fiap.tds.services;

import fiap.tds.dtos.SearchListDto;
import fiap.tds.dtos.usuarioDto.CreateUsuarioDto;
import fiap.tds.dtos.usuarioDto.ResponseUsuarioDto;
import fiap.tds.entities.AutenticaUsuario;
import fiap.tds.entities.Usuario;
import fiap.tds.exceptions.BadRequestException;
import fiap.tds.exceptions.NotFoundException;
import fiap.tds.infrastructure.DatabaseConfig;
import fiap.tds.repositories.AutenticaUsuarioRepository;
import fiap.tds.repositories.CidadeRepository;
import fiap.tds.repositories.UsuarioRepository;

import java.sql.SQLException;
import java.time.LocalDateTime;
import java.util.List;

public class UsuarioService {


    private final UsuarioRepository usuarioRepository = new UsuarioRepository();
    private final AutenticaUsuarioRepository autenticaUsuarioRepository = new AutenticaUsuarioRepository();
    private final CidadeRepository cidadeRepository = new CidadeRepository();

    // Registrar novo usuário
    public void registrar(CreateUsuarioDto dto) {

        // Verificando se o objeto possui todos os campos necessários
        if (dto.nomeUsuario() == null || dto.telefoneContato() == null || dto.autenticaUsuario() == null) {
            throw new BadRequestException("Todos os campos obrigatórios devem ser preenchidos.");
        }

        // Verificando se a senha tem no mínimo 8 caracteres
        if (dto.autenticaUsuario().getSenhaUsuario().length() < 8 || dto.autenticaUsuario().getSenhaUsuario().length() > 30) {
            throw new BadRequestException("A senha deve ter no mínimo 8 e no máximo 30 caracteres");
        }

        // Verificando se o email já está em uso
        if (autenticaUsuarioRepository.validarEmailExistente(dto.autenticaUsuario().getEmailUsuario())) {
            throw new BadRequestException("O email informado já está cadastrado");
        }

        // Valiação do tipo
        if (!dto.tipoUsuario().equalsIgnoreCase("CLIENTE") && !dto.tipoUsuario().equalsIgnoreCase("COLABORADOR")) {
            throw new BadRequestException("O tipo de usuário deve ser CLIENTE ou COLABORADOR");
        }

        // Adicionar validação de telefone

        // Validação da cidade
        var cidadeExistente = cidadeRepository.buscarPorId(dto.idCidade());

        if (cidadeExistente == null){
            throw new NotFoundException("A cidade informada não existe ou foi deletada.");
        }

        // Convertendo DTO para objeto
        var usuario = new Usuario();
        usuario.setNomeUsuario(dto.nomeUsuario());
        usuario.setTipoUsuario(dto.tipoUsuario());
        usuario.setTelefoneContato(dto.telefoneContato());
        usuario.setIdCidade(dto.idCidade());
        usuario.setAutenticaUsuario(dto.autenticaUsuario());
        usuario.setDataCriacao(LocalDateTime.now());
        usuario.setDeleted(false);

        usuarioRepository.registrar(usuario);
    }

    // Função para buscar os usuários com lógica de filtros
    public SearchListDto<ResponseUsuarioDto> buscar(int page, String nome, String tipo, String direction) {
        if (page < 1) {
            throw new BadRequestException("O número da página deve ser maior ou igual a 1.");
        }
        final int PAGE_SIZE = 9;
        var resultado = usuarioRepository.buscar(nome, tipo, direction);
        // Informações de paginação
        int totalItems = resultado.totalItems();
        List<Usuario> usuarios = resultado.data();
        int start = Math.max((page - 1) * PAGE_SIZE, 0);
        int end = Math.min(start + PAGE_SIZE, totalItems);
        // Criando a lista que será retornada
        List<ResponseUsuarioDto> pageData = List.of();
        if (start < totalItems) {
            pageData = usuarios.subList(start, end).stream()
                    .map(this::converterUsuarioParaDto)
                    .toList();
        }
        return new SearchListDto<>(page, direction, PAGE_SIZE, totalItems, pageData);
    }

    // Função para buscar usuario por ID
    public ResponseUsuarioDto buscarPorId(int id) {
        // Validação da existência do usuario
        var usuarioExistente = usuarioRepository.buscarPorId(id);
        if (usuarioExistente == null) {
            throw new NotFoundException("Colaborador com o ID " + id + " não encontrado");
        }
        return converterUsuarioParaDto(usuarioExistente);
    }



    // Atualizar cidade de preferência do usuário
    public void atualizarCidade(int id, CreateUsuarioDto dto){
        var usuarioExistente = usuarioRepository.buscarPorId(id);

        // Validação da existência do usuário que será atualizado
        if (usuarioExistente == null) {
            throw new NotFoundException("Usuário não encontrado");
        }

        // Validação dos campos
        if (dto.idCidade() <= 0) {
            throw new BadRequestException("O ID da cidade deve ser maior que zero.");
        }

        var cidadeExistente = cidadeRepository.buscarPorId(dto.idCidade());

        if (cidadeExistente == null){
            throw new NotFoundException("Cidade não encontrada ou deletada.");
        }

        // Convertendo o DTO em entidade
        var usuario = new Usuario();
        usuario.setIdCidade(dto.idCidade());

        usuarioRepository.atualizarCidadeUsuario(id, usuario);
    }


    // Função para atualizar o email de acesso de um usuário
    public void atualizarEmailUsuario(int idUsuario, CreateUsuarioDto dto) {
        var usuarioExistente = usuarioRepository.buscarPorId(idUsuario);

        if (usuarioExistente == null) {
            throw new NotFoundException("Usuário não encontrado com o ID: " + idUsuario);
        }

        if (dto.autenticaUsuario() == null) {
            throw new BadRequestException("Dados de autenticação não fornecidos para atualização de e-mail.");
        }

        var novoEmail = dto.autenticaUsuario().getEmailUsuario();

        // Validação do novo email
        if (novoEmail == null || novoEmail.isBlank()) {
            throw new BadRequestException("O novo e-mail não pode ser vazio.");
        }

        var emailRegex = "^[a-zA-Z0-9_+&*-]+(?:\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\.)+[a-zA-Z]{2,7}$";
        if (!novoEmail.matches(emailRegex)) {
            throw new BadRequestException("Formato de e-mail inválido.");
        }

        usuarioRepository.atualizarEmailUsuario(idUsuario, novoEmail);
    }


    // Função para atualizar a senha de acesso de um usuário
    public void atualizarSenhaUsuario(int idUsuario, CreateUsuarioDto dto) {
        var usuarioExistente = usuarioRepository.buscarPorId(idUsuario);

        if (usuarioExistente == null) {
            throw new NotFoundException("Usuário não encontrado com o ID: " + idUsuario);
        }

        if (dto.autenticaUsuario() == null) {
            throw new BadRequestException("Dados de autenticação não fornecidos para atualização de senha.");
        }

        var novaSenha = dto.autenticaUsuario().getSenhaUsuario(); // Acesso como record

        // Validação da nova senha
        if (novaSenha == null || novaSenha.isBlank()) {
            throw new BadRequestException("A nova senha não pode ser vazia.");
        }

        if (novaSenha.length() < 8 || novaSenha.length() > 30) {
            throw new BadRequestException("A senha deve ter entre 8 e 30 caracteres.");
        }

        // A senha é passada diretamente, sem hashing, conforme solicitado.
        usuarioRepository.atualizarSenhaUsuario(idUsuario, novaSenha);
    }

    // Função para atualizar o telefone de contato de um usuário
    public void atualizarTelefoneUsuario(int idUsuario, CreateUsuarioDto dto) {
        var usuarioExistente = usuarioRepository.buscarPorId(idUsuario);

        if (usuarioExistente == null) {
            throw new NotFoundException("Usuário não encontrado com o ID: " + idUsuario);
        }

        var novoTelefone = dto.telefoneContato();

        // Validação do novo telefone
        if (novoTelefone == null || novoTelefone.isBlank()) {
            throw new BadRequestException("O telefone de contato não pode ser vazio.");
        }

        String telefoneApenasNumeros = novoTelefone.replaceAll("\\D", "");

        if (!telefoneApenasNumeros.matches("^\\d{10,11}$")) {
            throw new BadRequestException("Formato de telefone inválido. Deve conter 10 ou 11 dígitos numéricos (ex: DDD + número).");
        }

        usuarioRepository.atualizarTelefoneUsuario(idUsuario, telefoneApenasNumeros);
    }

    // Atualizar usuario (nome e tipo)
    public void atualizar(int id, CreateUsuarioDto dto) {
        var usuarioExistente = usuarioRepository.buscarPorId(id);
        // Validação da existência do usuario que será atualizado
        if (usuarioExistente == null) {
            throw new NotFoundException("Nenhum usuario encontrado");
        }
        // Validação dos campos
        if (dto.nomeUsuario() == null || dto.telefoneContato() == null) {
            throw new BadRequestException("Todos os campos obrigatórios devem ser preenchidos.");
        }

        // Adicionar validação de CIDADE


        // Convertendo o DTO em entidade
        var usuario = new Usuario();
        usuario.setDeleted(false);
        usuario.setNomeUsuario(dto.nomeUsuario());
        usuario.setTelefoneContato(dto.telefoneContato());
        usuario.setIdCidade(dto.idCidade());
        usuario.setDataCriacao(LocalDateTime.now());

        usuarioRepository.atualizar(id, usuario);
    }

    // Função para deletar usuarioes
    public void deletar(int id) {
        Usuario usuarioExistente = usuarioRepository.buscarPorId(id);

        if (usuarioExistente == null) {
            throw new NotFoundException("Nenhum usuário encontrado");
        }
        usuarioRepository.deletar(id);
    }

    // Função que converte Usuario em ResponseUsuarioDto
    private ResponseUsuarioDto converterUsuarioParaDto(Usuario usuario) {
        if (usuario == null) {
            // Normalmente, se o usuário for nulo, uma NotFoundException já teria sido lançada
            // antes de chamar este método. Mas é uma boa prática verificar.
            return null;
        }

        String emailUsuario = null;
        AutenticaUsuario autenticacao = usuario.getAutenticaUsuario();
        if (autenticacao != null && autenticacao.getEmailUsuario() != null && !autenticacao.getEmailUsuario().isBlank()) {
            emailUsuario = autenticacao.getEmailUsuario();
        }

        return new ResponseUsuarioDto(
                usuario.getIdUsuario(),
                usuario.isDeleted(),
                usuario.getDataCriacao(),
                usuario.getNomeUsuario(),
                usuario.getTipoUsuario(),
                usuario.getIdCidade(),
                usuario.getTelefoneContato(),
                emailUsuario
        );
    }
}
